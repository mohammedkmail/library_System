...
class ReservationService {
    ...
    Reservation createReservation(User user, Book book) {
        ...
        if (existing) {
            throw new IllegalStateException('لديك حجز فعال لهذا الكتاب بالفعل.')
        }
    ...
    Reservation assignCopyToReservation(Long reservationId, Long bookCopyId) {
        ...
        if (!reservation) {
            throw new IllegalArgumentException('الحجز غير موجود.')
        }

        if (reservation.status != 'WAITING') {
            throw new IllegalStateException(
                'يمكن تجهيز الحجوزات الموجودة في قائمة الانتظار فقط.'
            )
        }
        ...
        if (!bookCopy) {
            throw new IllegalArgumentException('نسخة الكتاب غير موجودة.')
        }
    ...
    Reservation updateFulfillmentPreference(
        Long id,
        String method,
        ...
        if (reservation.status != 'READY') {
            throw new IllegalStateException(
                'هذا الحجز غير جاهز للدفع حاليًا.'
            )
        }

        String normalized =
            method?.toUpperCase() in ['PICKUP', 'DELIVERY'] ?
                method.toUpperCase() :
                'PICKUP'
    ...
    Reservation confirmPayment(Long id) {
        ...
        if (reservation.status != 'READY') {
            throw new IllegalStateException(
                'الحجز لم يعد جاهزًا للدفع.'
            )
        }

        if (
            !reservation.assignedCopy ||
            reservation.assignedCopy.status != 'RESERVED'
        ) {
            throw new IllegalStateException(
                'النسخة المخصصة لهذا الحجز لم تعد محفوظة. راجع إدارة المكتبة.'
            )
        }
        ...
        reservation.readyUntil = daysFromNow(3)
    ...
    Reservation fulfillReservation(Long id) {
        ...
        if (reservation.status != 'PAID') {
            throw new IllegalStateException(
                'يجب تأكيد دفع الحجز قبل تسليم الكتاب.'
            )
        }
    ...
    Reservation cancelReservation(
        ...
        if (
            ...
            throw new IllegalStateException(
                'الحجز مدفوع؛ تواصل مع الإدارة لمعالجة الإلغاء والاسترداد.'
            )
        }
    ...
    private Reservation prepareReservation(
        ...
        reservation.feeAmount =
            reservation.book?.borrowingFee ?: BigDecimal.ZERO
    ...
    private void releaseCopyAndContinueQueue(
        Reservation reservation
    ) {
        BookCopy releasedCopy =
            reservation.assignedCopy
    ...
    private void validateAvailableCopy(
        ...
        if (!book || !bookCopy) {
            throw new IllegalArgumentException(
                'الكتاب والنسخة مطلوبان.'
            )
        }

        if (bookCopy.book?.id != book.id) {
            throw new IllegalArgumentException(
                'النسخة المختارة لا تتبع هذا الكتاب.'
            )
        }

        if (bookCopy.status != 'AVAILABLE') {
            throw new IllegalStateException(
                'هذه النسخة غير متاحة حاليًا.'
            )
        }
    }
